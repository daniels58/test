...
@client.event
async def on_ready():
    await client.change_presence(game=Game(name="answering your questions"))
    print("logged in")
    print("Online")
    print("---------")  
...
async def on_message (message):
        ...
        if message.content.upper() == "HI":
            await client.send_message(message.channel,"hi")   #hi

        if message.content.upper() == "WEBSITE":
            await client.send_message(message.channel,"https://dojrrp.wixsite.com/gtav")    

        if message.content.upper() == "ALIVE":
            await client.send_message(message.channel,"Hey guys I am back alive! try some commands like 'Hi', 'joke', 'emoji' etc.")
        ...
        if message.content.upper().startswith("WHO ARE YOU"):
            await client.send_message(message.channel, "I am just a Bot who answer questions")

        if message.content.upper().startswith("LOL"):
            await client.send_message(message.channel, "lol")    

        if message.content.upper().startswith("CODE RP TIME"):
            await client.send_message(message.channel, "@everyone please join pre lobby briefing for a roleplay")

        if message.content.upper().startswith("HELP CAD"):
            await client.send_message(message.channel, "please go to the link and sign up https://dojrrp-cadmdt.bubbleapps.io/. Access password 'S1A1R1R1P1")   

        if message.content.upper().startswith("WHEN IS THE ROLEPLAY"):
        ...
        if message.content.upper().startswith('CURRENT SCHEDULE'):
            ...
            embed.add_field(name="Sorry current schedule is currently unavailable", value="OLD SCHEDULE", inline=False)
        ...
        if message.content.upper().startswith('HELP'):
            embed = discord.Embed(title="!Help command", description="everything you need to know is in this command", color=0x00ff00)
            embed.add_field(name="Hi", value="This command will greet you", inline=False)
            await client.send_message(message.channel, embed=embed)

        if message.content.upper().startswith('UNIFORM'):
            embed = discord.Embed(title="Roleplay Schedule", description=".", color=0x00ff00)
            embed.add_field(name="•Sunday- Recruiting Day", value="gvvhbhbmm nknk", inline=True)
            await client.send_message(message.channel, embed=embed)

                

                  
client.run('NDY5ODg1Mjk1MDYwNDUxMzI5.DxQkiA.F4G6aT26GPQJwnHL-acDVkONEdA')
